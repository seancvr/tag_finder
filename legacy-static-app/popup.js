...
// Find tags button
document.querySelector("#find-tags").addEventListener("click", async () => {
  ...
  let newUnmatchedUrlList = scriptData.srcUrls.filter(
    (url) => getTagId(url, idTagRegex) === null
  );
...
gtagList.addEventListener("click", async (e) => {
  ...
  tagData = tagData.filter((obj) => obj.pageUrl !== url);
